 ...
 * Handles multi-line cells and uses configurable newline characters.
 */
public class CsvTableFormatter {
    ...
    /**
     * Creates a formatter with default newline character ("\n")
     */
     ...
     * Headers are derived from the first row if available.
     *
     * @param rows the data rows to format
     * @param limit maximum number of rows to display, or -1 for unlimited
     * @return formatted table string, or empty string if rows is null or empty
     */
    public String formatTable(List<Row> rows, int limit) {
        ...
        List<Row> rowsToDisplay = limit < 0 ? rows : rows.stream().limit(limit).toList();

        // Calculate column widths
        int[] columnWidths = calculateColumnWidths(rowsToDisplay, headers);
